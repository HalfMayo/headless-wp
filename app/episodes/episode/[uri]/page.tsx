import { getEpisodes, getPost } from "@/app/lib/data";
...
export default async function Page({ params }: { params: { uri: string } }) {
  ...
  const prevEp = await getEpisodes(5);
      ...
      <article className="flex flex-col gap-8 sm:items-start items-center justify-center w-screen sm:w-[80vw] sm:my-[20vh]">
        ...
        <div className="flex sm:flex-row flex-col-reverse sm:gap-16 gap-8">
          <div className="flex flex-col sm:w-[20vw] w-screen gap-4">
            <div className="dark:bg-base-dark bg-base-light text-black sm:rounded-3xl p-8">
              <h3 className="font-semibold text-xl mb-4">Last episodes</h3>
              <ul className="list-none divide-y">
                {prevEp.map((ep: PostSum) => (
            ...
            <div className="flex flex-col gap-4 sm:rounded-3xl py-6 px-8 dark:bg-accent-dark bg-accent-light">
              <h3
          ...
          <div className="flex flex-col gap-8 text-lg px-8 sm:pr-0 sm:pl-2 sm:text-md">
            ...
            <Link href="/episodes">&larr; Back to the episodes list</Link>
          </div>
        </div>
      </article>
    </main>
